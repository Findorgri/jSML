...
/**
 * Class to calculate the CRC16 check sum
 */
public class Crc16 {
     ...
     */
    private static final int[] table = { 0x0000, 0x1189, 0x2312, 0x329b, 0x4624, 0x57ad, 0x6536, 0x74bf, 0x8c48, 0x9dc1,
     ...
     * @return crc16 value
     */
    public static int check(byte[] bytes, int length) {
        int crc = 0xffff;
    ...
    public static int check(byte[] bytes) {
        return check(bytes, bytes.length);
    }

    static int tableIndexAt(int index) {
        return table[index];
    }
